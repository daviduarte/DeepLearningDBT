...
public:
   ...
   explicit BackprojectOp(OpKernelConstruction* context) : OpKernel(context) {
	...
	printf("\n\n VALOR DO ISO[] %f, %f, %f", iso[0], iso[1], iso[2]);
      ...
      for( int n = 0; n < N_; ++n )
      {
         ...
         persp += gt(n,2,0) * iso[0] + gt(n,2,1) * iso[1] + gt(n,2,2) * iso[2];	// Esse é o h
         persp = 1 / persp;	// Esse é o h


	printf("\n\nPRINTANDO A NOVA MATRIZ DE PROJEÇÃO NORMALIZADA!!!!! ************");
         // loop over matrix
         for( int i = 0; i < 3; ++i )
         {
            for( int j = 0; j < 4; ++j )
...
public:
   ...
   void Compute(OpKernelContext* context) override
   {
      // grab input
      const auto proj = context->input( 0 ).tensor<float, 3>();
//      auto meu_proj = context->input( 0 ).tensor<float, 3>();
      // TODO: Check that #projections == #matrices
...
//   std::cout << proj << "\t";
      ...
      auto volume = volume_tensor->tensor<float, 3>();
        ...
        printf("CHEGOU AKIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII");
//        printf("asdsad %f", proj(0,0,0) );


//        std::cout << meu_proj(0, 0, 0) << ",";
//	auto array = meu_proj.data();
//	float* float_array = static_cast<float*>(array);

//printf("noizzz %f ", proj.data()[0][0][0]);
