...
REGISTER_OP("Backproject")
   ...
   .Attr("displacement: float")
   .Attr("pixel_dimen: tensor")
   .Output("vol: float")
...
protected: 
   ...
   // Displacement of breast in the world cordinates
   float displacement;

   // Detector pixel dims
   TensorShape pixel_shape_;
   float px_, py_;
...
public:
   ...
   explicit BackprojectOp(OpKernelConstruction* context) : OpKernel(context) {
      ...
      OP_REQUIRES_OK( context, context->GetAttr( "displacement", &displacement ) );
      //auto displacement = displacement_.tensor<1>();
      //displacement = displacement.dimension(0)


      Tensor p;
      OP_REQUIRES_OK( context, context->GetAttr( "pixel_dimen", &p ) );
      auto ps = p.tensor<float, 1>();
      px_ = ps( 0 );
      py_ = ps( 1 );
...
//      cout << 0 << "th output for class "<<0<<" is "<< vol(0, 0) <<end; 
