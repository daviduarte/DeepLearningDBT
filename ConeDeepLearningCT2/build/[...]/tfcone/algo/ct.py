...
def _backproject_grad( op, grad ):
    proj = project(
            ...
            pixel_dimen = op.get_attr( "pixel_dimen" )
        )
    return [ proj ]
...
class Reconstructor:
    ...
    def __init__( self, config, angles, displacement, trainable = False, name = None,
        weights_type = 'parker'):
        self.config = config
        ...
        with tf.name_scope( self.name, "Reconstruct" ) as scope:
            with tf.variable_scope( self.name, "Reconstruct" ):
                ...
                self.parker_w = tf.Variable(
                        initial_value = self.parker_w_np, #tf.random_uniform((15,1,2048), 0, 1), #self.parker_w_np,
                        dtype = tf.float32,
                        name = 'parker-weights',
                ...
                self.pixel_dimen_proto = tf.contrib.util.make_tensor_proto(
...
#                proj = tf.Print(proj, [self.parker_w], message="\n\n**Parker: ", summarize=10000)
#                proj = tf.Print(proj, [tf.reduce_min(self.parker_w)], message="\n\n**Valor mínimo do Parker: ")
#                proj = tf.Print(proj, [tf.reduce_max(self.parker_w)], message="**Valor maximo do Parker: ")
...
#                proj = tf.Print(proj, [tf.math.reduce_min(proj)], message="\n\n\n**************PRINTANDO VALOR MINIMO ANTES DA BACKPROJ ***************\n\n\n")
                ...
                vol = backproject(
                        ...
                        pixel_dimen  = self.pixel_dimen_proto,
                        name         = 'backproject'
                    )
...
#                vol = tf.Print(vol, [tf.math.reduce_min(vol)])
                ...
                #min = tf.reduce_min(vol)
                #vol = tf.add(vol, tf.abs(min))
